...
public class GuiProtocolSelector extends GuiScreen {
    ...
    class SlotList extends GuiSlot {
        ...
        protected void drawSlot(int slotIndex, int xPos, int yPos, int heightIn, int mouseXIn, int mouseYIn, float partialTicks) {
            drawCenteredString(mc.fontRenderer,(ViaLoadingBase.getInstance().getTargetVersion().getIndex() == slotIndex ? TextFormatting.GREEN.toString() + TextFormatting.BOLD : TextFormatting.GRAY.toString()) + ViaLoadingBase.getProtocols().get(slotIndex).getName(), width / 2, yPos + 2, -1);
            GlStateManager.pushMatrix();
