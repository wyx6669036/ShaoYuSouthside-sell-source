...
public class ManeWindow extends LiveWindow {
    ...
    ManeWindow() {
        ...
        liveProfile.username = AuthenticationStatus.INSTANCE.user.getName();
        liveProfile.uuid = Minecraft.getMinecraft().player.getUniqueID().toString();
        liveSkinUtil = new LiveSkinUtil(Minecraft.getMinecraft().player.getUniqueID());
        closeButton = false;
    ...
    public void mouseClicked(int mouseX, int mouseY, int mouseButton) {
        ...
        if (mouseInRect(w - buddyListX - scrollBarWidth, buddyListY, scrollBarWidth, h - (buddyListY + 10 + footer), mouseX, mouseY) && buddyListEntries.size() > 1) {
            ...
            if (i < buddyListEntries.size() && i >= 0) {
                ...
                if (buddyListEntry.online)
                    LivemessageGui.openChatWindow(buddyListEntry.username);
            }
        }
    ...
    public static void generateBuddylist() {
        ...
        final var channels = Chat.getInstance().getChannels();
        buddyListEntries.add(new BuddyListEntry(UUID.randomUUID(), "Channels", false));
        for (ChatChannel channel : channels) {
            buddyListEntries.add(new BuddyListEntry(UUID.randomUUID(), channel.getName(), true));
        }
    }
    ...
    public void drawBuddylist() {
        ...
        for (int i = listScrollPosition; i < buddyListEntries.size(); ++i) {
            if (lineHeight * 12 > h - (buddyListY + footer + 22))
                break;
            BuddyListEntry buddyListEntry = buddyListEntries.get(i);
            String buddyText = (buddyListEntry.uuid == null ? "\u00A7l" : ((buddyListEntry.online ? "   " : "   \u00A7o"))) + buddyListEntry.username;
            ...
            if (buddyListEntry.uuid != null) {
                ...
                if (unreads > 0)
                    fontRenderer.drawString(unreadString, buddyListX + 5 + fontRenderer.getStringWidth(buddyText + " "), buddyListY + 5 + 12 * lineHeight, getRGB(255, 0, 0));
            }
            lineHeight++;
        }
    }
    ...
    public void drawWindow(int bgColor, int fgColor) {
        ...
        textureManager.bindTexture(new ResourceLocation("southside/icons.png"));
