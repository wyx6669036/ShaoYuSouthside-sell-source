...
public abstract class TextHud extends BasicHud {
    ...
    public Dropdown textType = new Dropdown(
            "Text Type",
            new String[]{"No Shadow", "Shadow", "Full Shadow"},
            "",
            1,
            0
    );

    public TextHud(float x, float y, float scale, boolean background, boolean rounded, float cornerRadius, float paddingX, float paddingY, OneColor bgColor, boolean border, float borderSize, OneColor borderColor) {
    ...
    protected void drawLine(String line, float x, float y, float scale) {
        TextRenderer.drawScaledString(line, x, y, color.getValue().getRGB(), TextRenderer.TextType.toType(textType.getValue()), scale);
    }
