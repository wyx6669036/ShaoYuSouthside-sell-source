...
public class ConfigSlider extends BasicOption implements IFocusable {
    ...
    public ConfigSlider(Field field, Object parent, String name, String description, String category, String subcategory, float min, float max, int step) {
        this(field, parent, name, description, category, subcategory, min, max, step, false, 0.1F);
    }

    public ConfigSlider(Field field, Object parent, String name, String description, String category, String subcategory, float min, float max, int step, boolean instant, float increment) {
    ...
    public void draw(long vg, int x, int y, InputHandler inputHandler) {
        ...
        if (dragging) {
            ...
            value = (float) MathUtils.mapIncrement(xCoordinate, x + 352, x + 864, min, max, increment);
...
//                    isFloat = false;
        ...
        if (stepSlideAnimation.isFinished() && lastSliderTarget != -1 && lastSliderTarget != xCoordinate && (!dragging || startedDragging || step > 0) && lastX == x) {
            ...
            stepSlideAnimation = new EaseInOutCubic(Preferences.INSTANCE.trackerResponseDuration.getValue().intValue(), 0f, 1f, false);
        }
