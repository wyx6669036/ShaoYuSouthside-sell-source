...
import dev.diona.southside.module.modules.combat.Velocity;
import dev.diona.southside.module.modules.movement.KeepSprint;
...
{
    public Vec3d blinkHitPos = null;
    public int blinkHitCount = 0;
    ...
    public void onUpdate()
    {
        ...
        if (!Blink.isInstanceEnabled() || blinkHitPos != null && this.getDistanceSq(blinkHitPos.x, blinkHitPos.y, blinkHitPos.z) >= 36) {
            blinkHitPos = null;
            blinkHitCount = 0;
        }
    ...
    public void onDeath(DamageSource cause)
    {
        ...
        if (cause != null)
        {
            this.motionX = (double)(-MathHelper.cos((this.attackedAtYaw + this.movementYaw) * 0.017453292F) * 0.1F);
            this.motionZ = (double)(-MathHelper.sin((this.attackedAtYaw + this.movementYaw) * 0.017453292F) * 0.1F);
        }
        else
        {
            this.motionX = 0.0D;
    ...
    public EntityItem dropItem(ItemStack droppedItem, boolean dropAround, boolean traceItem)
    {
        ...
        else
        {
            ...
            {
                float f2 = 0.3F;
                entityitem.motionX = (double)(-MathHelper.sin(this.movementYaw * 0.017453292F) * MathHelper.cos(this.rotationPitch * 0.017453292F) * f2);
                entityitem.motionZ = (double)(MathHelper.cos(this.movementYaw * 0.017453292F) * MathHelper.cos(this.rotationPitch * 0.017453292F) * f2);
                ...
                if (f > 0.0F || f1 > 0.0F)
                {
                    ...
                    if (flag5)
                    {
                        if (i > 0)
                        {
                            if (targetEntity instanceof EntityLivingBase)
                            {
                                ((EntityLivingBase)targetEntity).knockBack(this, (float)i * 0.5F, (double)MathHelper.sin(this.movementYaw * 0.017453292F), (double)(-MathHelper.cos(this.movementYaw * 0.017453292F)));
                            }
                            else
                            {
                                targetEntity.addVelocity((double)(-MathHelper.sin(this.movementYaw * 0.017453292F) * (float)i * 0.5F), 0.1D, (double)(MathHelper.cos(this.movementYaw * 0.017453292F) * (float)i * 0.5F));
                            }

                            if (!KeepSprint.isInstanceEnabled() && !Velocity.attacked) {
                                this.motionX *= 0.6D;
                                this.motionZ *= 0.6D;
                        ...
                        if (flag3)
                        {
                            ...
                            for (EntityLivingBase entitylivingbase : this.world.getEntitiesWithinAABB(EntityLivingBase.class, targetEntity.getEntityBoundingBox().grow(1.0D, 0.25D, 1.0D)))
                            {
                                ...
                                {
                                    entitylivingbase.knockBack(this, 0.4F, (double)MathHelper.sin(this.movementYaw * 0.017453292F), (double)(-MathHelper.cos(this.movementYaw * 0.017453292F)));
