...
public abstract class Entity implements ICommandSender
{
    ...
    public boolean handleWaterMovement()
    {
        ...
        else if (!NoWeb.isEnable() && this.world.handleMaterialAcceleration(this.getEntityBoundingBox().grow(0.0D, -0.4000000059604645D, 0.0D).shrink(0.001D), Material.WATER, this))
        {
    ...
    {
        return this.world.isMaterialInBB(this.getEntityBoundingBox().grow(-0.10000000149011612D, -0.4000000059604645D, -0.10000000149011612D), Material.LAVA) && !NoWeb.isEnable();
    }
    ...
    public void moveRelative(float strafe, float up, float forward, float friction)
    {
        ...
        if (this == mc.player) {
            Southside.eventBus.post(strafeEvent);
        ...
        if (f >= 1.0E-4F)
        {
            ...
            up = up * f;
            forward = forward * f;
            float f1 = MathHelper.sin(strafeEvent.getYaw() * 0.017453292F);
            float f2 = MathHelper.cos(strafeEvent.getYaw() * 0.017453292F);
    ...
    {
        if (yaw == this.rotationYaw && pitch == this.rotationPitch) {

            LookEvent lookEvent = new LookEvent(new dev.diona.southside.util.player.Rotation(yaw, pitch));
            Southside.eventBus.post(lookEvent);

            yaw = lookEvent.getRotation().yaw;
            pitch = lookEvent.getRotation().pitch;
        }
    ...
    @Nullable
    public RayTraceResult rayTraceCustom(double blockReachDistance, float yaw, float pitch, float partialTicks) {
        final Vec3d vec3 = this.getPositionEyes(partialTicks);
        final Vec3d vec31 = this.getVectorForRotation(pitch, yaw);
        final Vec3d vec32 = vec3.add(vec31.x * blockReachDistance, vec31.y * blockReachDistance, vec31.z * blockReachDistance);
        return this.world.rayTraceBlocks(vec3, vec32, false, false, true);
    }

    @Nullable
    public RayTraceResult rayTraceCustom(double blockReachDistance, float yaw, float pitch) {
        return this.rayTraceCustom(blockReachDistance, yaw, pitch, 1F);
    }
