...
import dev.diona.southside.event.events.JumpEvent;
import dev.diona.southside.event.events.YawUpdateEvent;
import dev.diona.southside.module.modules.combat.AutoGapple;
import dev.diona.southside.module.modules.movement.FastLadder;
import dev.diona.southside.module.modules.movement.Flight;
import dev.diona.southside.module.modules.movement.StrafeFix;
import dev.diona.southside.module.modules.player.NoJumpDelay;
...
public abstract class EntityLivingBase extends Entity {
    ...
    public boolean isOnLadder() {
        ...
        FastLadder fastLadder = (FastLadder) Southside.moduleManager.getModuleByClass(FastLadder.class);
        if (fastLadder.isEnabled() && fastLadder.cancel) {
            return false;
        }
    ...
    private int getArmSwingAnimationEnd() {
        if (this.isPotionActive(MobEffects.HASTE)) {
            ...
            return this.isPotionActive(MobEffects.MINING_FATIGUE) ? 6 + (1 + this.getActivePotionEffect(MobEffects.MINING_FATIGUE).getAmplifier()) * 2 : OldHitting.isOldHitting() ? (int) (6 * OldHitting.getSwingSpeedValue()) : 6;
        }
    }
    ...
    protected void jump() {
        ...
        Southside.eventBus.post(jumpEvent);
...
//        this.motionY = (double)this.getJumpUpwardsMotion();
        ...
        if (this instanceof EntityPlayerSP) {
            if (RotationUtil.targetRotation != null && StrafeFix.doFix()) {
                this.movementYaw = RotationUtil.targetRotation.yaw;
                this.velocityYaw = RotationUtil.targetRotation.yaw;
            }
        }
        if (this.isSprinting()) {
            float fixedYaw = this.rotationYaw;
            if (RotationUtil.targetRotation != null && StrafeFix.doFix()) {
                fixedYaw = RotationUtil.targetRotation.yaw;
            }
//            if (Sprint.getSprint()) {
//                fixedYaw += MovementUtil.getMovingYaw() - this.rotationYaw;
//            }
...
//            this.motionZ += (double)(MathHelper.cos(f) * 0.2F);
            ...
            if (!this.isInWater() || this instanceof EntityPlayer && ((EntityPlayer) this).capabilities.isFlying) {
                if (!this.isInLava() || this instanceof EntityPlayer && ((EntityPlayer) this).capabilities.isFlying) {
                    if (this.isElytraFlying()) {
                        ...
                        if (this.collidedHorizontally && (!this.world.isRemote && !(this instanceof EntityPlayerSP))) {
                        ...
                        if (this.onGround && (!this.world.isRemote && !(this instanceof EntityPlayerSP))) {
                        ...
                        if (this.isPotionActive(MobEffects.LEVITATION)) {
                            ...
                            if ((!this.world.isRemote && !(this instanceof EntityPlayerSP)) || this.world.isBlockLoaded(blockpos$pooledmutableblockpos) && this.world.getChunk(blockpos$pooledmutableblockpos).isLoaded()) {
    ...
    public void onUpdate() {
        ...
        if (this.swingProgress > 0.0F) {
            final YawUpdateEvent event = new YawUpdateEvent(rotationYaw);
            if (this instanceof EntityPlayerSP) {
    ...
    protected float updateDistance(float p_110146_1_, float p_110146_2_) {
        ...
        float f1 = MathHelper.wrapDegrees(event.getYaw() - this.renderYawOffset);
        ...
        this.renderYawOffset = event.getYaw() - f1;
