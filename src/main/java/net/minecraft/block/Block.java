...
import dev.diona.southside.event.events.BlockBBEvent;
import dev.diona.southside.module.modules.misc.ExperimentalFeatures;
...
public class Block {
    ...
    public boolean shouldSideBeRendered(IBlockState blockState, IBlockAccess blockAccess, BlockPos pos, EnumFacing side) {
        ...
        if (xray.isEnabled() && !xray.getCaveOnly().getValue() && xray.getShowing().contains(pos)) {
            for (EnumFacing facing : EnumFacing.values()) {
                if (!originalShouldSideBeRendered(blockState, blockAccess, pos.offset(facing), facing)) continue;
                return true;
            }
        }
        return originalShouldSideBeRendered(blockState,blockAccess, pos, side);
    }
    ...
    protected static void addCollisionBoxToList(BlockPos pos, AxisAlignedBB entityBox, List<AxisAlignedBB> collidingBoxes, @Nullable AxisAlignedBB blockBox) {
        if (ExperimentalFeatures.INSTANCE.isEnabled() && mc.player != null && mc.player.getDistanceSq(pos) >= 100) {
            return;
        }
        if (blockBox != NULL_AABB) {
            ...
            if (world != null) {
                BlockBBEvent event = new BlockBBEvent(pos, axisalignedbb);
    ...
    /**
     * Called after a player destroys this Block - the posiiton pos may no longer hold the state indicated.getBlockBoundsMaxX
     */
    ...
    public BlockRenderLayer getRenderLayer() {
        var xray = (Xray) Southside.moduleManager.getModuleByClass(Xray.class);
        return xray.isEnabled() ? (this instanceof BlockOre || this instanceof BlockRedstoneOre) ? BlockRenderLayer.SOLID : BlockRenderLayer.TRANSLUCENT : BlockRenderLayer.SOLID;
    }
