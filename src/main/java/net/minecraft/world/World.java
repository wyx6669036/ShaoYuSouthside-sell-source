...
public abstract class World implements IBlockAccess
{
                    ...
                    if ((!flag2 || !flag3) && this.isBlockLoaded(blockpos$pooledmutableblockpos.setPos(k1, 64, l1)))
                    {
                        for (int i2 = k; i2 < l; ++i2)
                        {
                            if (!flag2 && !flag3 || i2 != l - 1)
                            {
                                ...
                                if (ExperimentalFeatures.INSTANCE.isEnabled() && mc.player != null && mc.player.getDistanceSq(blockpos$pooledmutableblockpos) >= 100) continue;
    ...
    public void updateEntityWithOptionalForce(Entity entityIn, boolean forceUpdate)
    {
        ...
        if (forceUpdate && entityIn.addedToChunk)
        {
            ...
            if (mc.playerStuckTicks > 0 && entityIn == mc.player) {
                mc.playerStuckTicks--;
