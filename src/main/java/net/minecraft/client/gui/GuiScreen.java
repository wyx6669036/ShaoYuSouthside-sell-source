...
public abstract class GuiScreen extends Gui implements GuiYesNoCallback
{
    ...
    private final void drawGradientRect(float left, float top, float right, float bottom, int startColor, int endColor) {
        ...
        buffer.pos(right, top, 0D).color(f1, f2, f3, f).endVertex();
        buffer.pos(left, top, 0D).color(f1, f2, f3, f).endVertex();
        buffer.pos(left, bottom, 0D).color(f5, f6, f7, f4).endVertex();
        buffer.pos(right, bottom, 0D).color(f5, f6, f7, f4).endVertex();
        tessellator.draw();
