...
public class GuiPlayerTabOverlay extends Gui {
    ...
    public String getPlayerName(NetworkPlayerInfo networkPlayerInfoIn) {
        ...
        if (networkPlayerInfoIn.liveUser != null) {
            name = LiveComponent.getLiveUserDisplayName(networkPlayerInfoIn.liveUser) + " " + name;
        }

        // Hacker Detection
        if (HackerDetector.reportedPlayers.contains(networkPlayerInfoIn.getGameProfile().getName())) {
            name = TextFormatting.LIGHT_PURPLE + "[SilenceFix] " + TextFormatting.RESET + name;
        }
        if (Southside.moduleManager.getModuleByClass(HackerDetector.class).isEnabled()) {
            PlayerData playerData = ((HackerDetector) Southside.moduleManager.getModuleByClass(HackerDetector.class)).getPlayData(networkPlayerInfoIn.getGameProfile().getId());
            if (playerData != null && playerData.getVl() > 5) {
                name = String.format(TextFormatting.RED + "[Hacker] %s", TextFormatting.RESET + name);
            }
        }
