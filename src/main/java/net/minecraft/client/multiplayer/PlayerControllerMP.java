...
import dev.diona.southside.event.events.ClickWindowEvent;
...
public class PlayerControllerMP {
    ...
    public void updateController() {
        ...
        if (this.connection.getNetworkManager().isChannelOpen()) {
            ...
            if (!this.connection.getNetworkManager().disconnected || !Southside.moduleManager.getModuleByClass(SilentDisconnect.class).isEnabled())
    ...
    public EnumActionResult processRightClick(EntityPlayer player, World worldIn, EnumHand hand) {
        if (this.currentGameType == GameType.SPECTATOR) {
            ...
            this.syncCurrentPlayItem();
            if (mc.player.getHeldItemMainhand().getItem() instanceof ItemSword && !OldHitting.isBlocking()) {
//                if (OldHitting.giveShield()) {
//                    mc.player.inventory.offHandInventory.set(0, new ItemStack(Items.SHIELD));
//                }
                NoSlow.severSideBlocking = true;
                this.connection.sendPacket(new CPacketPlayerTryUseItem(EnumHand.OFF_HAND));
                OldHitting.setBlocking(true);
            } else {
    ...
    public void attackEntity(EntityPlayer playerIn, Entity targetEntity) {
        ...
        AttackEvent preEvent = new AttackEvent(targetEntity, EventState.PRE);
        ...
        this.syncCurrentPlayItem();
        CPacketUseEntity packet = new CPacketUseEntity(targetEntity);
        packet.critical = playerIn.fallDistance > 0.0F && !playerIn.onGround && !playerIn.isOnLadder() && !playerIn.isInWater() && !playerIn.isPotionActive(MobEffects.BLINDNESS) && !playerIn.isRiding() && targetEntity instanceof EntityLivingBase;
        ...
        AttackEvent postEvent = new AttackEvent(targetEntity, EventState.POST);
    ...
    public ItemStack windowClick(int windowId, int slotId, int mouseButton, ClickType type, EntityPlayer player) {
        ClickWindowEvent event = new ClickWindowEvent(windowId, slotId, mouseButton, type);
