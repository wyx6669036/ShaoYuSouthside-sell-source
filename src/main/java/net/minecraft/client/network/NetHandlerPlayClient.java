...
import dev.diona.southside.module.modules.misc.SilentDisconnect;
...
public class NetHandlerPlayClient implements INetHandlerPlayClient
{
    ...
    public void handleJoinGame(SPacketJoinGame packetIn)
    {
        ...
        HytProtocol.sendRegisterPacket();
    ...
    public void handleEntityVelocity(SPacketEntityVelocity packetIn)
    {
        ...
        if (Disabler.processingFakePlayers && entity instanceof EntityPlayer player) {
            entity = Disabler.getOrSpawnFakeEntity(player);
        }
    ...
    public void handleSpawnPlayer(SPacketSpawnPlayer packetIn)
    {
        ...
        //FIND ME :: OptimaMC
        if(this.getPlayerInfo(packetIn.getUniqueId()) == null) return;
    ...
    public void handlePlayerPosLook(SPacketPlayerPosLook packetIn)
    {
        ...
        // NoRotate Start
        if (Objects.requireNonNull(Southside.moduleManager.getModuleByName("NoRotate")).isEnabled() && mc.player != null && mc.player.ticksExisted > 60) {
    ...
    {
        if (Southside.moduleManager.getModuleByClass(SilentDisconnect.class).isEnabled()) {
            mc.ingameGUI.addChatMessage(ChatType.SYSTEM, new TextComponentString(I18n.format("disconnect.lost")));
            mc.ingameGUI.addChatMessage(ChatType.SYSTEM, new TextComponentString(reason.getFormattedText()));
        } else {
    ...
    public void sendPacketNoHigherEvent(Packet<?> packetIn) {
        this.netManager.sendPacketNoHigherEvent(packetIn);
    }
    ...
    {
        ChatEvent e = new ChatEvent(packetIn.getType().getId(), packetIn.getChatComponent());
        Southside.eventBus.post(e);
        PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.client);
    ...
    public void handleConfirmTransaction(SPacketConfirmTransaction packetIn)
    {
        ...
        {
            CPacketConfirmTransaction packet = new CPacketConfirmTransaction(packetIn.getWindowId(), packetIn.getActionNumber(), true);
    ...
    public void handleTeams(SPacketTeams packetIn)
    {
        ...
        if (packetIn.getAction() == 4 && scoreplayerteam != null)
        {
