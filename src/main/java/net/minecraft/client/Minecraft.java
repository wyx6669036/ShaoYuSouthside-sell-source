...
import dev.diona.southside.util.render.WallpaperEngine;
...
//    public final Queue < FutureTask<? >> scheduledTasks = Queues.newArrayDeque();
    ...
    public Minecraft(GameConfiguration gameConfig) {
        ...
        this.username = Southside.user != null ? Southside.user.trim() : "DefaultPlayer";
        logger.info("Setting user: " + this.username);
        if (Southside.user == null) {
            logger.warn("Southside.user is null, using session username as fallback");
            this.username = this.session.getUsername();
        } else {
            this.username = Southside.user.trim();
        }
        this.username = Southside.user != null ? Southside.user.trim() : "";
...
//    @VirtualizationLock
    private void init() throws LWJGLException, IOException {
        ...
        // Initialize WallpaperEngine
        wallpaperEngine = new WallpaperEngine();
        String gameDirPath = Southside.MC.mc.gameDir.getAbsolutePath();
        String videoFilePath = gameDirPath + File.separator + "bg.mp4";
        try {
            InputStream inputStream = Southside.class.getResourceAsStream("/assets/minecraft/southside/video/bg.mp4");
            Files.copy(inputStream, Paths.get(videoFilePath), StandardCopyOption.REPLACE_EXISTING);
        } catch (IOException e) {
            e.printStackTrace();
        }

        wallpaperEngine.setup(new File(videoFilePath), 120);
    ...
    public void displayGuiScreen(@Nullable GuiScreen guiScreenIn) {
        if (LiveClient.INSTANCE.liveUser == null && !(guiScreenIn instanceof GuiAuthentication)) {
            return;
        }
...
//            }
    ...
    public void loadWorld(@Nullable WorldClient worldClientIn, String loadingMessage) {
        ...
        } else if (!isSingleplayer() && player != null) {
            LiveClient.INSTANCE.sendPacket(LiveProto.createUpdateMinecraftProfile(
                    EntityPlayer.getUUID(player.getGameProfile()),
                    player.getName()
            ));
        }
    }
