...
public class BlockModelRenderer {
    ...
    public boolean renderModel(IBlockAccess worldIn, IBakedModel modelIn, IBlockState stateIn, BlockPos posIn, BufferBuilder buffer, boolean checkSides, long rand) {
        ...
        boolean flag = (Minecraft.isAmbientOcclusionEnabled() && stateIn.getLightValue() == 0 && modelIn.isAmbientOcclusion()) || xray.isEnabled();
        ...
        try {
            ...
            var block = stateIn.getBlock();

            if (xray.isEnabled() && xray.getOutline().getValue()) {
                for (EnumFacing facing : EnumFacing.values()) {

                    if (modelIn.getQuads(null, facing, 0L).isEmpty()) continue;

                    var x = posIn.getX();
                    var y = posIn.getY();
                    var z = posIn.getZ();

                    var another = new BlockPos(x, y, z);

                    if (checkSides && !stateIn.shouldSideBeRendered(worldIn, posIn.offset(facing), facing)
                            || !(block instanceof BlockOre || block instanceof BlockRedstoneOre)
                            || xray.getDistanceXZ(x, z) > xray.getRange().getValue().doubleValue()
                            || xray.getMap().containsKey(another))
                        continue;

                    xray.getMap().put(another, block);
                }
            }
