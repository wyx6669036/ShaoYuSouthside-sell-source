...
import dev.diona.southside.module.modules.combat.SmartBlock;
                            ...
                            if (OldHitting.isOldHitting()) {
                                ...
                                switch (OldHitting.getMode()) {
                                    ...
                                    case "Leaked":
                                        ...
                                        final float convertedProgress = MathHelper.sin(MathHelper.sqrt(p_187457_5_) * (float) Math.PI);
                                        transformSideFirstPersonBlock(enumhandside, p_187457_7_, 0.0F);
...
//                    this.transformFirstPerson(enumhandside, p_187457_5_);
    ...
    private void debug(float equipProgress, float swingProgress) {
        GlStateManager.translate(0.56, -0.52, -0.72);
        GlStateManager.translate(-0.1, 0.1, 0.1414214);
        ...
        GlStateManager.rotate((float) (f1 * 35.0F), -8.0F, -5F, -5.0F);
        GlStateManager.rotate((float) (f1 * 10.0F), 2.0F, 0F, 5.5F);
    }
...
//        GlStateManager.scale(OldHitting.Scale.get(), OldHitting.Scale.get(), OldHitting.Scale.get());
    ...
    public void sigmaold(EnumHandSide p_187459_1_, float equippedProg, float swingProgress) {
        int side = p_187459_1_ == EnumHandSide.RIGHT ? 1 : -1;
...
//        GlStateManager.scale(OldHitting.scaleValue.get(), OldHitting.scaleValue.get(), OldHitting.scaleValue.get());
    ...
    public void jello(EnumHandSide p_187459_1_, float equippedProg, float swingProgress) {
        int side = p_187459_1_ == EnumHandSide.RIGHT ? 1 : -1;
        GlStateManager.translate(0.56F, -0.52F, -0.71999997F);
        GlStateManager.rotate(-102.25F, 1, 0, 0);
        GlStateManager.rotate(side * 13.365F, 0, 1, 0);
...
//        GlStateManager.scale(OldHitting.scaleValue.get(), OldHitting.scaleValue.get(), OldHitting.scaleValue.get());
    ...
    public void Zoom(EnumHandSide p_187459_1_, float equippedProg, float swingProgress) {
        int side = p_187459_1_ == EnumHandSide.RIGHT ? 1 : -1;
        GlStateManager.translate(0.56F, -0.52F, -0.71999997F);
