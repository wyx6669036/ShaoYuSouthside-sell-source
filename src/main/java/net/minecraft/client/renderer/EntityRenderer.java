...
import dev.diona.southside.module.modules.combat.Reach;
...
public class EntityRenderer implements IResourceManagerReloadListener
{
    ...
    private final BezierUtil zoomAnimation = new BezierUtil(4, 1);
...
//                if (movingObjectPosition != null) d1 = movingObjectPosition.hitVec.distanceTo(vec3d);
            ...
            if (this.pointedEntity != null && flag && vec3d.distanceTo(vec3d3) > Reach.getReach())
            {
                this.pointedEntity = null;
    ...
    {
        if (Config.zoomMode) {
            zoomAnimation.update(4);
        } else {
            zoomAnimation.update(1);
        }
        ...
        else
        {
            ...
            if (flag)
            {
                if (!Config.zoomMode)
                {
                    ...
                    this.mc.gameSettings.smoothCamera = SmoothZoom.enableSmoothCamera();
                    this.mc.renderGlobal.displayListEntitiesDirty = true;
                }

                if (Config.zoomMode)
                {
                    f /= SmoothZoom.doSmooth() ? zoomAnimation.get() : 4F;
                }
            }
    ...
    private void hurtCameraEffect(float partialTicks)
    {
        ...
        {
            if (Southside.moduleManager.getModuleByClass(NoHurtCam.class).isEnabled()) return;
    ...
    public void orientCamera(float partialTicks)
    {
        ...
        if (entity instanceof EntityLivingBase && ((EntityLivingBase)entity).isPlayerSleeping())
        {
            ...
            if (!this.mc.gameSettings.debugCamEnable)
            {
                ...
                GlStateManager.rotate(Perspective.getCameraPrevYaw() + (Perspective.getCameraYaw() - Perspective.getCameraPrevYaw()) * partialTicks + 180.0F, 0.0F, -1.0F, 0.0F);
                GlStateManager.rotate(Perspective.getCameraPrevPitch() + (Perspective.getCameraPitch() - Perspective.getCameraPrevPitch()) * partialTicks, -1.0F, 0.0F, 0.0F);
            }
        }
...
//                    float f16 = this.mc.gameSettings.gammaSetting;
                    float f16 = Southside.moduleManager.getModuleByClass(Xray.class).isEnabled() ? 10000 : this.mc.gameSettings.gammaSetting;
    ...
    public void updateCameraAndRender(float partialTicks, long nanoTime)
    {
        ...
        if (!this.mc.skipRenderWorld)
        {
            ...
            if (LiveClient.INSTANCE.liveUser != null) {
                clientUsername = LiveClient.INSTANCE.liveUser.getName();
            } else {
                clientUsername = "Nobody";
            }

            NanoVGHelper instance = NanoVGHelper.INSTANCE;
            instance.setupAndDraw(true, vg -> {
                var fontSize = 10;
                instance.drawTextWithFormatting(vg, clientUsername, scaledresolution.getScaledWidth() - instance.getTextWidth(vg, clientUsername, fontSize, Fonts.WQY), scaledresolution.getScaledHeight() - instance.getTextHeight(vg, fontSize, Fonts.WQY) / 2, -1, fontSize, Fonts.WQY);
            });
        }
